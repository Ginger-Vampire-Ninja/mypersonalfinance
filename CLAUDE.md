...
**IDs** are cryptographically random integers generated by `genId()` (`crypto.getRandomValues` — 53-bit, fits JS `Number` and PostgreSQL `int8`, no schema changes needed). Never use `Date.now()` for IDs — it was replaced for unpredictability. **Dates** are always `YYYY-MM-DD` strings; always parse with `new Date(ds + 'T00:00:00')` to avoid timezone shifts.
